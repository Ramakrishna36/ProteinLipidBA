...
def load_trained_model(model: BaseModel, model_configs: ModelConfigs, is_training: bool) -> BaseModel:
    ...
    for name, param in checkpoint.items():
        if (
            ...
            or "lipid_model.base_model.model.encoder.layer" in name
        ) and (
    ...
    if model_configs.lipid_fine_tuning_type in [
        "lora",
        "lokr",
        "loha",
        "ia3",
    ]:
        print("Merging lipid model with its adapter")
...
def load_pretrained_pkd_bounds(checkpoint_path: str) -> Tuple[float, float]:
    ...
    elif "proteinlipid" in checkpoint_path:
        # Added bounds for protein-lipid interactions
        pkd_lower_bound = 2.0
        pkd_upper_bound = 10.0
    else:
